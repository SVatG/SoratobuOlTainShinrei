...
void DrawHorizontalLineNoClip(Bitmap *bitmap,int x,int y,int len,Pixel c)
{
	...
	for(int i=0;i<len;i++) DrawPixel(bitmap,x+i,y,c);
}
...
{
	for(int i=0;i<len;i++) DrawPixel(bitmap,x+i,y,comp(ReadPixel(bitmap,x+i,y),c));
}
...
{
	for(int i=0;i<len;i++) DrawPixel(bitmap,x,y+i,c);
}
...
{
	for(int i=0;i<len;i++) DrawPixel(bitmap,x,y+i,comp(ReadPixel(bitmap,x,y+i),c));
}
